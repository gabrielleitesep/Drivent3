...
describe("GET /hotels/:hotelId", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 200 and list all hotel rooms", async () => {
      const user = await createUser();
      const token = await generateValidToken(user);
      const enrollment = await createEnrollmentWithAddress(user);
      const ticketType = await createHotelTicket();
      const ticket = await createTicket(enrollment.id, ticketType.id, TicketStatus.PAID);
      const databaseHotel = await createHotel()
      const databaseRooms = await createRooms(databaseHotel.id)
      const response = await server.get(`/hotels/${databaseHotel.id}`).set("Authorization", `Bearer ${token}`);
      ...
      expect(response.body).toEqual([{
        ...
        Rooms: [{
          id: databaseRooms.id,
          name: databaseRooms.name,
          capacity: databaseRooms.capacity,
          hotelId: databaseHotel.id,
          createdAt: databaseRooms.createdAt.toISOString(),
          updatedAt: databaseRooms.updatedAt.toISOString(),
        }]
      }]);
    });
  });
